...
Event Description: Core Competency: Sub-competency (Score) – short rationale ... [1–2 paragraph description using the member's rank and last name on first mention, then only they/them/their pronouns thereafter.]

Outcome: [2–3 sentence measurable or strategic result.]

Note:

Use only the format "Communication: Written Communication (HE) – ..." for competencies.
...
response = client.chat.completions.create( model="gpt-4", messages=[ { "role": "system", "content": "You are a military admin assistant trained to write professional performance feedback notes." }, { "role": "user", "content": prompt } ] ) output_text = response.choices[0].message.content st.markdown("### ✏️ Generated Feedback Note") st.text_area("Output", output_text, height=400) # Create Word Document def create_word_doc(text): doc = Document() doc.add_heading("Performance Feedback Note", 0) lines = text.strip().splitlines() for line in lines: line = line.strip() if line.lower().startswith("event description"): doc.add_paragraph("Event Description:", style='Heading 2') elif line.lower().startswith("outcome"): doc.add_paragraph("Outcome:", style='Heading 2') elif not line: doc.add_paragraph("") else: para = doc.add_paragraph() run = para.add_run(line) run.font.name = 'Arial' run._element.rPr.rFonts.set(qn('w:eastAsia'), 'Arial') run.font.size = Pt(11) buffer = BytesIO() doc.save(buffer) buffer.seek(0) return buffer doc_buffer = create_word_doc(output_text) st.download_button( label="📄 Download as Word Document", data=doc_buffer, file_name="feedback_note.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document" ) 

except Exception as e: st.error(f"Failed to connect to OpenAI API: {e}")
